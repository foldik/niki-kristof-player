...
public class LeftDiagonalEvaluatorTest {

	@Mock
	private Line line;
	
	private LeftDiagonalEvaluator leftDiagonalEvaluator;
	
	@Before
	public void setUp() {
		MockitoAnnotations.initMocks(this);
		leftDiagonalEvaluator = new LeftDiagonalEvaluator();
	}
	...
	public void testWithFourXAndOneSideWinner() {
		...
		List<Element> elements = Arrays.asList(
				...
				new Element(1, 1, BorderPiece.X),
				new Element(2, 2, BorderPiece.X),
				new Element(3, 3, BorderPiece.X),
				new Element(4, 4, BorderPiece.X)
				);
		when(line.getSubLines()).thenReturn(elements);
		
		//WHEN
		List<Coordinate> winnerCoordinates = leftDiagonalEvaluator.evaluate(line, BorderPiece.X);
		
		//THEN
		assertEquals(1, winnerCoordinates.size());
		
		assertEquals(new Coordinate(5, 5), winnerCoordinates.get(0));
	}
	...
	public void testWithFourXTwoSideWinner() {
		...
		assertEquals(new Coordinate(5, 5), winnerCoordinates.get(1));
	}
	...
	public void testWithOneXOneEmptyThreeX() {
		...
		assertEquals(new Coordinate(1, 1), winnerCoordinates.get(0));
	}
	...
	public void testWithFourXBothSidesClosed() {
		// GIVEN
		List<Element> elements = Arrays.asList(
				new Element(1, 1, BorderPiece.O),
				new Element(2, 2, BorderPiece.X),
				new Element(3, 3, BorderPiece.X),
				new Element(4, 4, BorderPiece.X),
				new Element(5, 5, BorderPiece.X),
				new Element(6, 6, BorderPiece.O)
				);
		when(line.getSubLines()).thenReturn(elements);
